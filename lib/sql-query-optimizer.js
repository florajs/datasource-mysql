 ...
 * @private
 */
const getAttributeExpressions = filterTree((node) => has(node, 'table') && node.table === null && node.type && node.type === 'column_ref');
...
function getRequiredTables(ast) {
    ...
    ['columns', 'where', 'groupby', 'orderby'].forEach((clause) => {
        // extract tables used in given clauses
        if (ast[clause] !== null) {
...
function getRequiredAliases(ast) {
    ...
    ['groupby', 'orderby'].forEach((clause) => {
        if (ast[clause]) {
            Array.prototype.push.apply(aliasExpressions, getAttributeExpressions(ast[clause]));
        }
    });
