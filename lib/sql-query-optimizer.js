 ...
 * @private
 */
const getTableExpressions = filterTree((node) => node.type && node.type === 'column_ref');
...
const getAttributeExpressions = filterTree(
    (node) => has(node, 'table') && node.table === null && node.type && node.type === 'column_ref'
);
...
function getRequiredTables(ast) {
    const tableExpressions = ['columns', 'where', 'groupby', 'orderby']
        .filter((clause) => has(ast, clause) && ast[clause] !== null)
        .map((clause) => ast[clause])
        // TODO: replace reduce/push by flatMap(getTableExpressions) when available
        .reduce((expressions, expr) => {
            expressions.push(...getTableExpressions(expr));
            return expressions;
        }, []);
...
function optimizeAST(ast, requestedAttrs, isLimitPer = false) {
    ...
    if (has(ast, '_next') && typeof ast._next === 'object') {
        return ast;
    }
