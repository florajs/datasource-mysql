...
var createBinaryExpr = nodeQuery.AstHelper.createBinaryExpr,
    OPERATOR_MAPPING = {
        equal: '=',
        notEqual: '!=',
        greater: '>',
...
SqlQueryBuilder.prototype.filter = function (filters) {
    ...
    self.ast.where = self.ast.where === '' ? conditions : createBinaryAndExpr(self.ast.where, conditions);
};
...
function mapSingleAttributeFilter(filter, operator, columns) {
    if (Array.isArray(filter.value)) {
        // array values and equal/notEqual operators are mapped to IN/NOT IN SQL expressions
...
function mapMultiAttributeFilter(filter, operator, columns) {
    ...
    expr.paren = true; // parenthesize complete expression
    return expr;
}
...
SqlQueryBuilder.prototype.search = function (attributes, term) {
    ...
    if (typeof this.ast.where === 'object') {
...
SqlQueryBuilder.prototype._getColumns = function () {
    ...
    this.ast.columns.forEach(function (column) {
        columns[column.as !== '' ? column.as : column.expr.column] = column.expr;
    });
    return columns;
};
