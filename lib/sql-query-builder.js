...
function mapSingleAttributeFilter(filter, operator, columns) {
    return createBinaryExpr(operator, columns[filter.attribute], operator === 'LIKE' ?
...
module.exports = function generateAST({
    filter, limitPer, queryAST, searchable, search, order, limit, page
}) {
    ...
    if ((filter && filter.length) ||
        (queryAST._meta && queryAST._meta.hasFilterPlaceholders)) qb.filter(filter);
