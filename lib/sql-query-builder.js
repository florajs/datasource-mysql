...
function mapSingleAttributeFilter(filter, operator, columns) {
    return createBinaryExpr(operator, columns[filter.attribute], operator === 'LIKE' ? `%${filter.value}%` : filter.value);
}
...
function mapMultiAttributeFilter(filter, operator, columns) {
    const expr = filter.value
        ...
        .map(values =>
            values.map((value, index) => createBinaryExpr(operator, columns[filter.attribute[index]], value)).reduce(createBinaryAndExpr)
        )
...
class SqlQueryBuilder {
    ...
    filter(filters) {
        ...
        if (!this.ast._meta || !this.ast._meta.hasFilterPlaceholders) {
            ...
            this.ast.where = this.ast.where === null ? conditions : createBinaryAndExpr(this.ast.where, conditions);
        } else {
     ...
     */
    limit(count, offset) {
        this.ast.limit = [{ type: 'number', value: offset }, { type: 'number', value: count }];
    }
